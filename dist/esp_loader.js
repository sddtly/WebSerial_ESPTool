...
export class ESPLoader extends EventTarget {
    ...
    async initialize() {
        if (!this._parent) {
            ...
            if (portInfo.usbVendorId && portInfo.usbProductId) {
                ...
                this.logger.log(`USB 串行芯片: ${chipInfo.name} (VID: 0x${portInfo.usbVendorId.toString(16)}, PID: 0x${portInfo.usbProductId.toString(16)})`);
        ...
        try {
            ...
            this.logger.debug(`USB 连接类型: ${this._isUsbJtagOrOtg ? "USB-JTAG/OTG" : "外部串行芯片"}`);
        }
        catch (err) {
        ...
        try {
            ...
            this.logger.debug(`USB 模式（寄存器）: ${usbMode.mode} (uartNo=${usbMode.uartNo})`);
        }
        catch (err) {
        ...
        this.logger.debug(`引导加载程序闪存偏移量: 0x${FlAddr.flashOffs.toString(16)}`);
        // 标记初始化成功
    ...
    async detectChip() {
        try {
            ...
            if (chipInfo) {
                ...
                this.logger.debug(`通过 IMAGE_CHIP_ID 检测到芯片: ${chipId} (${this.chipName})`);
                return;
            }
        ...
        if (chip === undefined) {
            throw new Error(`未知芯片: 十六进制: ${toHex(chipMagicValue >>> 0, 8).toLowerCase()} 数字: ${chipMagicValue}`);
        }
        ...
        this.logger.debug(`通过魔数检测到芯片: ${toHex(chipMagicValue >>> 0, 8)} (${this.chipName})`);
    }
    ...
    async getSecurityInfo() {
        ...
        if (responseData.length < 12) {
            throw new Error(`无效的安全信息响应长度: ${responseData.length}（至少需要 12 字节）`);
        }
    ...
    async connectWithResetStrategies() {
        ...
        for (const strategy of resetStrategies) {
            try {
                ...
                if (isUSBSerialChip) {
                    ...
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("同步超时")), 1000));
                    try {
        ...
        // 所有策略都失败 - 在抛出之前重置放弃标志
        this._abandonCurrentOperation = false;
        throw new Error(`无法同步到 ESP。请尝试手动复位。最后一个错误: ${lastError === null || lastError === void 0 ? void 0 : lastError.message}`);
    }
    /**
     * @name watchdogReset
    ...
    async rtcWdtResetChipSpecific() {
        ...
        if (this.chipFamily === CHIP_FAMILY_ESP32S2) {
            ...
            throw new Error(`rtcWdtResetChipSpecific() 不受支持于 ${this.chipFamily}`);
        }
        // 解锁看门狗寄存器
        ...
        try {
            ...
            if (isUsbJtagOrOtg) {
                ...
                try {
                    ...
                    this.logger.debug(`USB 模式: ${usbMode.mode} (uartNo=${usbMode.uartNo})`);
                }
                catch (err) {
                ...
                if (this.IS_STUB) {
                    ...
                    if (this.currentBaudRate !== ESP_ROM_BAUD) {
                        this.logger.debug(`将波特率从 ${this.currentBaudRate} 更改为 ${ESP_ROM_BAUD}`);
                    ...
                    if (this.currentBaudRate !== ESP_ROM_BAUD) {
                        this.logger.debug(`不在 stub 上，但波特率为 ${this.currentBaudRate} - 为 WDT 复位更改为 ${ESP_ROM_BAUD}`);
                        await this.reconfigurePort(ESP_ROM_BAUD);
                        this.currentBaudRate = ESP_ROM_BAUD;
                        this.logger.debug("波特率已更改为 115200");
                    }
                ...
                if (portWillChange) {
                    this.logger.debug(`WDT 复位后端口将更改 (${usbMode.mode}) - 需要端口重新选择`);
                    return true;
                }
                return false;
            }
        ...
        const executeCommand = async () => {
            ...
            if (this.IS_STUB || this.chipFamily == CHIP_FAMILY_ESP8266) {
                ...
                if (opcode === ESP_GET_SECURITY_INFO) {
                    ...
                    statusLen = 2;
                    this.logger.debug(`未知的芯片系列，默认使用 2 字节状态 (opcode: ${toHex(opcode)}, data.length: ${data.length})`);
                }
            }
    ...
    async readPacket(timeout) {
        ...
        if (this._isCDCDevice) {
                    ...
                    if (partialPacket === null) {
                        ...
                        if (byte == this.SLIP_END) {
                            ...
                            if (this.debug) {
                                ...
                                this.logger.debug("串行缓冲区中剩余数据: " + hexFormatter(this._inputBuffer));
                            }
                        ...
                        if (byte == this.SLIP_ESC_END) {
                            ...
                            throw new SlipReadError("无效的 SLIP 转义 (0xdb, " + toHex(byte) + ")");
                        }
                    }
            ...
            while (true) {
                ...
                if (this.debug)
                    this.logger.debug("读取 " + readBytes.length + " 字节: " + hexFormatter(readBytes));
    ...
    async setBaudrate(baud) {
        ...
        if (maxBaud && baud > maxBaud) {
            this.logger.log(`⚠️ 警告：波特率 ${baud} 超过 USB 串行芯片的限制 (${maxBaud})！`);
    ...
    async setBaudrateC5Rom(baud) {
        ...
        this.logger.log(`ROM 期望的晶振频率: ${crystalFreqRomExpect} MHz，检测到 ${crystalFreqDetect} MHz。`);
        let baudRate = baud;
    ...
    async flashData(binaryData, updateProgress, offset = 0, compress = false) {
        if (binaryData.byteLength >= 8) {
            ...
            this.logger.log(`镜像头部，Magic=${toHex(headerMagic)}，FlashMode=${toHex(headerFlashMode)}，FlashSizeFreq=${toHex(headerFlashSizeFreq)}`);
        }
        ...
        if (compress) {
            ...
            this.logger.log(`正在写入数据，文件大小: ${uncompressedFilesize}。压缩后大小: ${compressedFilesize}`);
        ...
        while (filesize - position > 0) {
            if (this.debug) {
                this.logger.log(`正在写入地址 ${toHex(offset + seq * flashWriteSize, 8)} `);
            }
        ...
        this.logger.log("写入 " + filesize + " 字节耗时 " + (Date.now() - stamp) + " 毫秒");
    ...
    async flashBegin(size = 0, offset = 0, encrypted = false) {
        ...
        if (size != 0 && !this.IS_STUB) {
            this.logger.log("擦除 " + numBlocks + " 字节耗时 " + (Date.now() - stamp) + " 毫秒");
        }
        return numBlocks;
    }
    /**
    ...
    async runSpiFlashCommand(spiflashCommand, data, readBits = 0) {
        ...
        if (dataBits == 0) {
            ...
            for (const word of words) {
                this.logger.debug(`将字 ${toHex(word)} 写入寄存器偏移量 ${toHex(nextReg)}`);
    ...
    async runStub(skipFlashDetection = false) {
        this.logger.debug(`正在为 ${this.chipName} 加载 stub，修订版本: ${this.chipRevision}`);
        const stub = await getStubCode(this.chipFamily, this.chipRevision);
        // 此芯片没有可用的 stub，返回 ROM 加载程序
        if (stub === null) {
            this.logger.log(`Stub 闪存器尚不支持 ${this.chipName}，使用 ROM 加载程序`);
            return this;
        }
        // 我们正在通过 USB 传输，对吗？
    ...
    async detectUsbConnectionType() {
        ...
        this.logger.debug(`USB-JTAG/OTG 检测: ${isUsbJtag ? "是" : "否"} (PID=0x${pid === null || pid === void 0 ? void 0 : pid.toString(16)})`);
        return isUsbJtag;
    }
    async getUsbMode() {
        var _a, _b;
    ...
    async enterConsoleMode() {
        ...
        try {
            ...
            this.logger.debug(`检测到 USB 连接类型: ${isUsbJtag ? "USB-JTAG/OTG" : "外部串行芯片"}`);
            ...
            if (this.isUsbJtagOrOtg === undefined) {
                throw new Error(`无法进入控制台模式：USB 连接类型未知且检测失败: ${err}`);
            }
    ...
    async _clearForceDownloadBootIfNeeded() {
        try {
            ...
            this.logger.debug(`${chipName} 强制下载启动寄存器: 0x${currentValue.toString(16)} (掩码: 0x${mask.toString(16)})`);
            // 检查标志是否已设置
    ...
    async _resetToFirmwareIfNeeded() {
        ...
        try {
            ...
            if (portWillChange) {
                this.logger.debug(`${this.chipName}：WDT 复位后端口将更改 - 用户必须重新选择端口`);
                // 触发事件以通知端口更改
    ...
    async reconnect() {
        ...
        try {
            ...
            if (!this.port.readable || !this.port.writable) {
                throw new Error(`打开后端口流不可用（可读: ${!!this.port.readable}，可写: ${!!this.port.writable}）`);
            }
            // 端口现在打开并准备就绪 - 允许初始化写入
            this._isReconfiguring = false;
            // 保存芯片信息和闪存大小（无需再次检测）
            ...
            if (savedBaudRate !== ESP_ROM_BAUD) {
                ...
                // 验证波特率更改后端口是否仍准备就绪
                if (!this.port.writable || !this.port.readable) {
                    throw new Error(`波特率更改后端口未准备就绪（可读: ${!!this.port.readable}，可写: ${!!this.port.writable}）`);
                }
            }
            // Stub 现在在芯片上运行
    ...
    async exitConsoleMode() {
        ...
        if (isUsbOtgChip && isUsbJtagOrOtg === undefined) {
            try {
                ...
                this.logger.debug(`USB 检测失败，假定 ${this.chipName} 为 USB-JTAG/OTG: ${err}`);
        ...
        if (isUsbOtgChip && isUsbJtagOrOtg) {
            ...
            // 等待复位完成并端口更改
            await sleep(500);
            this.logger.debug(`${this.chipName}：端口已更改。请选择引导加载程序端口。`);
            // 触发事件以通知端口更改
            this.dispatchEvent(new CustomEvent("usb-otg-port-change", {
                detail: {
                    chipName: this.chipName,
    ...
    async resetInConsoleMode() {
        ...
        if (!this.isConsoleResetSupported()) {
            this.logger.debug("ESP32-S2 USB-JTAG/CDC 不支持简单的控制台复位 - 使用 exitConsoleMode 进入引导加载程序");
            await this.exitConsoleMode();
            this.logger.debug("S2 现在处于引导加载程序模式 - 调用方必须在新端口上执行 syncAndWdtReset，然后重新连接控制台");
            return;
        }
        // 对于其他设备：使用标准固件复位
        try {
    ...
    async readFlash(addr, size, onPacketReceived, options) {
        ...
        // 在闪存读取操作之前刷新串行缓冲区
        await this.flushSerialBuffers();
        this.logger.log(`正在从闪存地址 0x${addr.toString(16)} 读取 ${size} 字节...`);
        ...
        if (this.isWebUSB()) {
            if (this._isCDCDevice) {
                ...
                this.logger.debug(`CDC 设备 - 已初始化: blockMultiplier=${this._adaptiveBlockMultiplier}, maxInFlightMultiplier=${this._adaptiveMaxInFlightMultiplier}`);
            }
            else {
        ...
        while (remainingSize > 0) {
            ...
            while (!chunkSuccess && retryCount <= MAX_RETRIES) {
                ...
                try {
                    // 仅在第一次尝试或重试时记录
                    if (retryCount === 0) {
                        this.logger.debug(`正在读取地址 0x${currentAddr.toString(16)} 的块，大小: 0x${chunkSize.toString(16)}`);
                    }
                    let blockSize;
                    let maxInFlight;
                    if ((options === null || options === void 0 ? void 0 : options.blockSize) !== undefined &&
                        ...
                        if (retryCount === 0) {
                            this.logger.debug(`使用自定义参数: blockSize=${blockSize}, maxInFlight=${maxInFlight}`);
                        }
                    }
                    else if (this.isWebUSB()) {
                    ...
                    while (resp.length < chunkSize) {
                        ...
                        try {
                            ...
                            if (err instanceof SlipReadError) {
                                this.logger.debug(`${err.message} 在字节 0x${resp.length.toString(16)} 处`);
                        ...
                        if (packet && packet.length > 0) {
                            ...
                            // 我们必须等待所有包再发送 ACK
                            const shouldAck = resp.length >= chunkSize || // 块结束
                    ...
                    if (this.isWebUSB() && this._isCDCDevice && retryCount === 0) {
                        ...
                        if (this._consecutiveSuccessfulChunks >= 2) {
                            ...
                            if (adjusted) {
                                ...
                                this.logger.debug(`速度提高: blockSize=${newBlockSize}, maxInFlight=${newMaxInFlight}`);
                    ...
                    if (this.isWebUSB() && this._isCDCDevice && retryCount === 1) {
                        ...
                        if (this._adaptiveBlockMultiplier > 1 ||
                            ...
                            const newBlockSize = baseBlockSize * this._adaptiveBlockMultiplier;
                            const newMaxInFlight = baseBlockSize * this._adaptiveMaxInFlightMultiplier;
                            this.logger.debug(`在较高速度下出错 - 降至最小值: blockSize=${newBlockSize}, maxInFlight=${newMaxInFlight}`);
                        }
                        else {
                            // 已在最小值且仍然失败 - 这是真正的错误
                            this.logger.debug(`在最低速度下出错 (blockSize=31, maxInFlight=31) - 非速度问题`);
                        }
                    }
                    // 检查是否为超时错误或 SLIP 错误
                    if (err instanceof SlipReadError) {
                        if (retryCount <= MAX_RETRIES) {
                            this.logger.debug(`已清空缓冲区并重试（尝试 ${retryCount}/${MAX_RETRIES}）...`);
                            // 继续重试相同的块（将发送新的读取命令）
                        }
                        else {
                            ...
                            if (!deepRecoveryAttempted) {
                                ...
                                this.logger.log(`在地址 0x${currentAddr.toString(16)} 处所有重试都已用尽。正在尝试恢复（关闭并重新打开端口）...`);
                                try {
                                    ...
                                    throw new Error(`在地址 0x${currentAddr.toString(16)} 处读取块失败，经过 ${MAX_RETRIES} 次重试且恢复失败: ${recoveryErr}`);
                                }
                            }
                            else {
                                // 已尝试恢复，放弃
                                throw new Error(`在地址 0x${currentAddr.toString(16)} 处读取块失败，经过 ${MAX_RETRIES} 次重试和恢复尝试`);
                            }
                        }
                    }
                    else {
            ...
            this.logger.debug(`总进度: 0x${allData.length.toString(16)} / 0x${size.toString(16)} 字节`);
        }
        return allData;
    }
}
class EspStubLoader extends ESPLoader {
    ...
    async memBegin(size, _blocks, _blocksize, offset) {
        ...
        this.logger.debug(`加载范围: ${toHex(load_start, 8)}-${toHex(load_end, 8)}`);
        this.logger.debug(`Stub 数据: ${toHex(stub.data_start, 8)}，长度: ${stub.data.length}，文本: ${toHex(stub.text_start, 8)}，长度: ${stub.text.length}`);
        for (const [start, end] of [
            [stub.data_start, stub.data_start + stub.data.length],
            [stub.text_start, stub.text_start + stub.text.length],
        ]) {
            if (load_start < end && load_end > start) {
                throw new Error("软件加载程序驻留在 " +
                    toHex(start, 8) +
                    "-" +
                    toHex(end, 8) +
                    "。 " +
    ...
    async eraseRegion(offset, size) {
        ...
        if (offset % FLASH_SECTOR_SIZE !== 0) {
            throw new Error(`偏移量 ${offset} (0x${offset.toString(16)}) 未与闪存扇区大小 ${FLASH_SECTOR_SIZE} (0x${FLASH_SECTOR_SIZE.toString(16)}) 对齐`);
        }
        if (size % FLASH_SECTOR_SIZE !== 0) {
            throw new Error(`大小 ${size} (0x${size.toString(16)}) 未与闪存扇区大小 ${FLASH_SECTOR_SIZE} (0x${FLASH_SECTOR_SIZE.toString(16)}) 对齐`);
        }
        // 检查合理范围（防止在 pack 中溢出）
        ...
        if (offset + size > maxValue) {
            throw new Error(`区域结束（偏移量 + 大小 = ${offset + size}）超过最大可寻址范围 ${maxValue}`);
        }
